...
CABINETS = {
    "lang":"Til Organish", "math":"Matematika va Aniq Fanlar",
    "chem":"Kimyo va Biologiya", "hum":"Gumanitar Fanlar",
    "soc":"Ijtimoiy Fanlar", "it":"IT va Dasturlash", "art":"Ijodiy Fanlar"
}
LANGUAGES = {
    "en":"Ingliz","ru":"Rus","de":"Nemis","fr":"Fransuz",
    "ar":"Arab","zh":"Xitoy","ja":"Yapon","ko":"Koreya",
    "es":"Ispan","tr":"Turk","uz":"Uzbek"
}
SUBJECTS = {
    ...
    "it":{"Python":"Python","Web":"Web dasturlash","AI":"Suniy intellekt"},
...
LEVEL_NAMES = {
    "beginner":"Boshlangich","elementary":"Asosiy",
    "intermediate":"Orta","upper":"Yuqori Orta",
    "advanced":"Ilgor","master":"Ustoz"
}
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Daraja: {level} | XP: {xp}\n{bar(xp%500,500)}\nStreak: {streak} kun\n\nBugun ham organamizmi?",
        reply_markup=menu_kb()
    )

async def main_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    user = q.from_user; st = db.get_user_stats(user.id)
...
async def handle_cabinet(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    cab = q.data.
[18/5/2026 1:29 pm] W: replace("cab_",""); uid = q.from_user.id
...
async def handle_level(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if choice=="lvl_zero":
        ...
        await q.edit_message_text(f"{subject} ni noldan boshlaymiz!\n\nAI sizni bosqichma-bosqich orgatadi!",
        ...
        if questions:
            ...
            db.set_knowledge_level(uid,"beginner"); ss["level"]="beginner"
            await q.edit_message_text("Boshlangich darajadan boshlaymiz!",
...
async def show_assess_q(q, ss):
    ...
    await q.edit_message_text(f"Bilim darajasi aniqlash\n{bar(i,total)}\nSavol {i+1}/{total}\n\n{qdata['question']}",
        reply_markup=InlineKeyboardMarkup(kb))
...
async def handle_assess(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ss["ai"]>=len(qs):
        ...
        db.
[18/5/2026 1:29 pm] W: set_knowledge_level(uid,level); ss["level"]=level
        await q.edit_message_text(
        ...
        await q.edit_message_text(f"{topic}\n\n{lesson}\n\n+10 XP!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Test",callback_data="do_quiz"),
                 InlineKeyboardButton("Keyingi",callback_data="next_topic")],
                [InlineKeyboardButton("AI dan sorash",callback_data="ai_chat"),
                 InlineKeyboardButton("Bosh sahifa",callback_data="main_menu")]]))
...
async def next_topic(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if idx>=len(topics):
        await q.edit_message_text("Barcha mavzularni tugatdingiz!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("MOOC Test",callback_data="mooc_menu")],
    ...
    await q.edit_message_text(f"{topic} ({idx+1}/{len(topics)})\n\n{lesson}\n\n+10 XP!",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Test",callback_data="do_quiz"),
             InlineKeyboardButton("Keyingi",callback_data="next_topic")],
            [InlineKeyboardButton("Bosh sahifa",callback_data="main_menu")]]))
  [18/5/2026 1:32 pm] W: async def do_quiz(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    uid = q.from_user.id; ss = s(uid)
...
[18/5/2026 1:32 pm] W: replace("ma_",""))
    qs=ss["mq"]; i=ss["mi"]
...
[18/5/2026 1:32 pm] W: game["start_time"]=datetime.now()
...
async def stop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid=update.
[18/5/2026 1:32 pm] W: effective_user.id; s(uid)["mode"]=None
